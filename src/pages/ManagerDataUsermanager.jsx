...
function ManagerDataUsermanager() {
  ...
  const { data: dataSegmentUsermanager, mutate: fetchDataSegmentUsermanager } =
    useSWR(`${API_DATA}/segment?SDT_UM=${user?.SDT}`);
  ...
  const handleSelectionChange = (e) => {
    ...
    const segment = dataSegmentUsermanager.find(
      (seg) => seg.MaPQ === e.currentKey
    );
    if (segment) {
  ...
  const { data: detailSegment, mutate: fetchDetailSegment } = useSWR(
    `${API_DATA}/segment/${segmentSelected}?lan=${contactSelected}`
  );
  ...
  const data = useMemo(() => {
    return (
      detailSegment?.map((customer, index) => {
        return {
          id: index + 1,
          sdt: customer?.SDT || "",
          ...
          <div className="relative flex gap-2">
            ...
            <Tooltip content="Edit user">
              <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                <Link
                  to={`/usermanager/data/edit/${user?.sdt}`}
                  className="text-lg text-default-400 cursor-pointer active:opacity-50"
                >
                  <EditIcon />
                </Link>
        ...
        <div className="headerContent flex mb-2">
          <div className="flex">
            <h1 className="titlePage">
              Danh sách dữ liệu {segmentSelected && segmentSelected}
            </h1>
            {/* <Chip
                            variant="flat"
                            color="success"
