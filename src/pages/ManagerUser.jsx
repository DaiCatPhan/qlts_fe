...
function ManagerUser() {
  ...
  const {
    ...
    `${API_USER}?name=${filterSearchName}&take=${pagination.take}&skip=${pagination.skip}`
  );
  ...
  const users = useMemo(() => {
    return (
      dataPagination?.data?.map((user, index) => {
        return {
          id: user?.TENDANGNHAP,
          role: user?.usermanager
            ? "usermanager"
            : user?.admin
              ? "admin"
              : "other",
          ...
          status:
            user?.usermanager?.TRANGTHAIUM || user?.admin?.TRANGTHAIADMIN || 0,
  ...
  const filteredItems = useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((users) =>
        users.sdt.toLowerCase().includes(filterSearchName.toLowerCase())
      );
      setRowsPerPage(100);
    } else {
      setRowsPerPage(5);
  ...
  const handleEXcel = () => {
    ...
    const data = dataPagination?.data?.map((item, index) => {
      let condition = {};
      if (item?.admin?.MAADMIN) {
        condition.HOTEN = item?.admin?.HOTEN;
        condition.DIENTHOAI = item?.admin?.SDT;
        condition.EMAIL = item?.admin?.EMAIL;
        condition.GENDER = item?.admin?.GIOITINH;
        condition.ROLE = "ADMIN";
      }

      if (item?.usermanager?.SDT) {
        condition.HOTEN = item?.usermanager?.HOTEN;
        condition.DIENTHOAI = item?.usermanager?.SDT;
        condition.EMAIL = item?.usermanager?.EMAIL;
        condition.GENDER = item?.usermanager?.GIOITINH;
        condition.ROLE = "USERMANAGER";
      }
      return {
        STT: index + 1,
        ...condition,
    ...
    excel.EX_Excel({ header, data, nameFile: "Danh sách người dùng" });
  };
